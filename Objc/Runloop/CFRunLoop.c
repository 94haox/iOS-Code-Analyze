...
struct __CFRunLoop {
    ...
    Boolean _unused;
    volatile _per_run_data *_perRunData;   // reset for runs of the run loop 预设状态
    pthread_t _pthread;
    uint32_t _winthread;
...
CF_INLINE void __CFRunLoopUnsetStopped(CFRunLoopRef rl) {
    rl->_perRunData->stopped = 0x0; // 将状态设置为 0
}
...
CF_INLINE void __CFRunLoopUnsetIgnoreWakeUps(CFRunLoopRef rl) {
    rl->_perRunData->ignoreWakeUps = 0x0; // 忽视唤起状态 0
}
...
// 全局字典 key 是 pthread, value 是 runloop 具体原因看👇
...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    ...
    if (!__CFRunLoops) {
        ...
        // 如果全局 dictionary不存在， 创建 runloop
        CFRunLoopRef mainLoop = __CFRunLoopCreate(pthread_main_thread_np());
        
        // 讲 runloop 存入字典作为value， key 是 pthread
    ...
    // 如果 t 不是 main thread 那么，注册回调，当线程销毁时，销毁对应的 runloop
    ...
    // 从👆代码可以看出，利用 dictionary 的特性 使得 runloop 和 pthread 是 一一对应的
    // 并且，在线程创建时， runloop 并没有创建， 只有在第一次获取时，才会创建，也就是我们经常用的懒加载
...
#pragma mark -
#pragma mark - 系统运行后调起主线程的 6 个主要函数
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    do {
        ...
        // 通知 observer 即将执行 timer 回调
        if (rlm->_observerMask & kCFRunLoopBeforeTimers) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeTimers);
        // 通知 observer 即将执行 source 回调
